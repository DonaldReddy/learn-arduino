...
- [x] Blink — onboard LED + external LED alternating (`basic/Blink`)
- [x] Pushbutton with debounce, `INPUT_PULLUP`, `millis()`-based timing (`basic/Button`)
- [x] Analog input — potentiometer via `analogRead`, printed to Serial (`basic/Potentiometer`)
- [x] PWM output — fade an LED with `analogWrite` (`basic/Potentiometer`, pot controls brightness)
